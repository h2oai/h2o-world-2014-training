...
gbm.h2o.fit <- h2o.gbm(training_frame = df, y = y, x = x, ntrees = 10)
...
safeSystem(cmd)
cmd <- sprintf("curl -o %s/GBMPojo.java http://%s:%d/2/GBMModelView.java?_modelKey=%s", tmpdir_name, myIP, myPort, model_id)
safeSystem(cmd)
cmd <- sprintf("curl -o %s/h2o-model.jar http://127.0.0.1:54321/h2o-model.jar", tmpdir_name)
safeSystem(cmd)
cmd <- sprintf("sed -i '' 's/class %s/class GBMPojo/' %s/GBMPojo.java", model_id, tmpdir_name)
safeSystem(cmd)
